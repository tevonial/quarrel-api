...
postRouter.get('/thread/:id', getPostsByThread);
...
function getPostsByThread(req, res, next) {
    /**
     * gets Posts by Thread, construct tree structure
     */
    // Post.find({thread: req.params.id, topLevel: true})
    //     .populate({
    //         path: 'author',
    //         select: 'name'
    //     }).exec((err, posts) => {
    ...
    // Thread.findById(req.params.id).populate({
    //         path: 'post'
    //     }
    // ).exec( (err, thread) => {
    //     if (err)    next(err);
